...
def setup_argv() -> argparse.Namespace:
    ...
    parser.add_argument("--overwrite", help="allow overwrite", default=False, action="store_true")
...
def check_file_exists(file_path: Path, overwrite: bool):
    if file_path.exists() and file_path.is_file():
        if overwrite:
            LOGGER.warning(f"file exists, overwriting")
            return False
        else:
            LOGGER.warning(f"file exists, skipping")
            return True
    else:
        return False
...
def main(argv: argparse.Namespace):
    ...
    if argv.ipp:
        ipp_path = getattr(argv, "hpp") / f"{filename_lower}.ipp"
    ...
    # write header
    hpp_path = getattr(argv, "hpp") / f"{filename_lower}.hpp"
    ...
    # write source
    cpp_path = getattr(argv, "cpp") / f"{filename_lower}.cpp"
