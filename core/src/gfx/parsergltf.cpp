...
void ParserGLTF::load_mesh(const std::size_t t_mesh_index)
{
  ...
  // TODO: REMOVE THIS and create a function to fill missing colors with default
  // also handle a case when color in unavailable from model
  ...
  const std::vector<Vertex> vertices{ assemble_vertices(positions, normals, colors) };
